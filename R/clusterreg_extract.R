#' Cluster Robust Regression Extract
#'
...
#' clustered.
#'
#' The default is estimator in `clusterreg_extract()` is CR2. Please see the
...
#' coerce to NA
#' @param .cluster_var  Character vector of column name for cluster variable
...
#' for linear model
#' @param .cr_method    Character vector of method desired for cluster
...
#' Dataframe must have the columns `Variable` and `Labels`
...
#' extract
#'
clusterreg_extract <- function(.dataset
                              ,.dependents
                              ,.treatment
  ...
  if(missing(.cluster_var)){
    stop("Cluster Variable Missing")
  }

  ##################################################################################
  # Subsetting Data to minimum necessary variables
  ##################################################################################
  ...
  ##################################################################################
  # Converting variables to factors if requested as factors
  ##################################################################################
  ...
  ##################################################################################
  # Create Data Frame of models to be run (per dependent)
  ##################################################################################
  ...
  variables_list$.cluster_var <- .cluster_var
  variables_list$.wt_var <- .wt_var
  variables_list$.cr_method <- .cr_method
  ...
  ##################################################################################
  # Calculating and joining summary statistics for input data
  ##################################################################################
  if (!is.na(.subgroup)) {
    UnadjStatsOverall <-
      mean_extract(
  ...
  ##################################################################################
  # Finalize Data: Add labels, trail, sample columns, and write to excel if requested
  ##################################################################################
  # if the labels are provided, join labels on
  if(!is.na(.dep_labels)){
    OutputDS <-
      OutputDS %>%
      dplyr::left_join(.dep_labels, by = dplyr::join_by(Dependent == Variable)) %>%
...
## Function is called on all formulas using pmap at line 161
clusterreg_helper <-
  function(.dataset
           ,.dependent
           ,.treatment
    ...
    # filtering out where dependent, cluster, weights, or subgroup variable is
    # missing. By default, the lm model would take care of missing values
    ...
    removed_clust_miss <- nrow(.dataset01a) - nrow(.dataset01b)

    if (removed_clust_miss > 0) {
      print(paste0(removed_clust_miss, " records were removed due to ",
                   "missing values in Cluster Variable: ",
                   .cluster_var, ". Please handle explicitly in data ",
                   "preparation if they should be included."))
    } # End checking for missing .cluster_var values
    ...
    if (!is.na(.wt_var)) {
      ...
      if (nrow(.dataset01b) != nrow(.dataset01c)) {
        print(paste0("Records were removed due to ",
    ...
    if (!missing(.by_values)) {
      ...
      if (is.na(.by_values)) {
        .dataset02 <- .dataset01c %>%
          dplyr::filter(is.na(!!sym(.subgroup)))
      } else {
        # otherwise, filter the data set to the level
        .dataset02 <- .dataset01c %>%
          dplyr::filter(!!sym(.subgroup) == {{.by_values}})
      } # end is.na(.by_values)
    } else {
      .dataset02 <- .dataset01c
    ...
    if(!is.na(.treatmentNA)){
      .dataset02 <-
        .dataset02 %>%
        dplyr::filter(!!rlang::ensym(.treatment) != .treatmentNA)
    }
  ...
  if (.cr_method == "match_SAS") {
    lmSE <- lmtest::coeftest(lmfit,
                             vcov = sandwich::vcovCL,
                             type = "HC1",
                             cluster = .dataset02[[.cluster_var]])
  } else {
    lmSE <- lmtest::coeftest(lmfit,
  ...
  Impact_SE <- lmSE %>%
    ...
    dplyr::rename_with(~ paste(c("Impact", "StdErr", "ProbT", "Stars"),
                               treatsuffix, sep = "_"),
                       c(estimate, std.error, p.value, stars)) %>%
    dplyr::bind_cols(Dependent = .dependent)
  ...
  VarStdDevG <-
    ...
    dplyr::summarise(StdDev = sd(!!rlang::ensym(.dependent))) %>%
    tidyr::pivot_wider(names_from = .treatment, values_from = "StdDev", names_prefix = "StdDev_")

  ## Combine necessary data frames and calculate
  ...
  ES <-
    ...
    dplyr::select(tidyselect::starts_with("EffectSize_"))
